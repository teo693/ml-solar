...
def prepare_input_sequences(data, sequence_length):
    sequences = []
    targets = []
...
def load_and_preprocess_data(data_path):
    try:
        ...
        start_time = time.time()
        scaler = MinMaxScaler()
        # Adjust columns for normalization
        data_to_normalize = data[['attributes_id', 'last_updated_ts']].values
        data_normalized = scaler.fit_transform(data_to_normalize)
        data[['attributes_id', 'last_updated_ts']] = data_normalized
        print("Data normalized. Time elapsed: {:.2f} seconds".format(time.time() - start_time))

        # Prepare input sequences
        sequence_length = 10  # Adjust as needed
        x, y = prepare_input_sequences(data[['attributes_id', 'last_updated_ts']].values, sequence_length)
